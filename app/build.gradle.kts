...
android {
    ...
    signingConfigs {
        create("release") {
            val ks = file(System.getenv("KEYSTORE_PATH") ?: "nonexistent")
            if (ks.exists()) {
                storeFile = ks
...
dependencies {
    ...
    // JSON parsing — kotlinx.serialization would add a plugin; org.json is already in Android SDK.
    // We'll use org.json which ships with Android. Zero added dependency.
}
